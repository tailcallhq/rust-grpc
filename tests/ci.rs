...
fn generate() {
    ...
    let build = Job::new("Build and Test")
        ...
        .add_step(Step::new("Install protoc").uses("arduino", "setup-protoc", "v3"))
        .add_step(
            Toolchain::default()
                .add_stable()
                .add_nightly()
    ...
    let workflow = Workflow::new("Build and Test")
        .add_env(flags)
        .on(event)
        .add_job("build", build)
        .add_job("deploy", deploy);

    // Normalize serializer whitespace while keeping generated CI checked in sync.
    let yaml = workflow.to_string().unwrap();
    let content = format!(
        "# @generated by gh-workflow (Rust) - do not edit by hand.\n{}\n",
        yaml.lines()
            .map(str::trim_end)
            .collect::<Vec<_>>()
            .join("\n")
